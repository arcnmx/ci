{ config, exec, env, cipkgs }: let
  scriptData = tasks'': with cipkgs.lib; let
    ...
    drvAttrs = fn: drvs: listToAttrs (map (drv: nameValuePair (exec.drvOf drv) (fn drv)) drvs);
    drvOrExec = drv: if drv ? ci.exec then executor.ci.executor.for drv else drv;
  in {
    # structured input data for buildScript
    drvs = map exec.drvOf drvs;
    drvExecutor = if executor == null then "" else executor.exec;
    drvTasks = map exec.drvOf (skippedTasks ++ tasks);
    ...
    drvNoCache = drvAttrs
      (drv: true)
      (filter (drv: ! drv.ci.cache or true)
        drvs);
    drvInputs = drvAttrs (drv: concatStringsSep " " (map (input: exec.drvOf (drvOrExec input)) (drv.inputs ++ drv.inputsSkipped))) (tasks ++ skippedTasks);
  ...
  script = ''
    ...
    if ! opRealise "''${CI_DRV_DIRTY[@]}" --show-trace --keep-going "$@" || [[ -n $OPT_DRY ]]; then
        ...
        if drv_valid $drv || ! drv_dirty $drv; then
          ...
          for input in ''${drvInputs[$drv]}; do
            if drv_valid $input || ! drv_dirty $input; then
              ...
              if [[ -z $OPT_DRY ]]; then
                nix-store -r $input --dry-run 2>&1 | (grep -vFe 'these derivations will be built' -e "$input" | sed -n '/^these paths will be fetched/q;p' >&2 || true)
