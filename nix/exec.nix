{ config, env, tasks }: let
  ...
  logpipe = msg: " | (${coreutils}/bin/tee -a /dev/stderr && echo ${msg} >&2)";
  ...
  in {
    ...
    buildCommand = ''
      ...
      ssh -i $executor/$prefix/$(basename $commandDrv) -o UserKnownHostsFile=$PWD/known_hosts -p $port $user@$address $commandDrv
    '';
  };
  commandExecutor = {
    drv
  ...
  , connectionDetails # address ? "127.0.0.1", port, user
    ...
    drv = cipkgs.stdenvNoCC.mkDerivation {
      ...
      sshdScript = ''
        #!@runtimeShell@
        @openssh@/bin/sshd -ef @out@/@prefix@/sshd_config -o "PidFile $EX_PIDFILE"
      '';
...
in {
  inherit buildAndRun toNix drvImports opFilter opFilterNoisy opRealise drvOf colours execSsh;
  buildTask = task: let
