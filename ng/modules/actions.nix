{ callPackage, config, lib, ... }: with lib; let
...
in {
  ...
  config = {
    configFile = callPackage ({ shellCommand, yq'build }: shellCommand {
      name = "gh-actions.yml";
      ...
      passAsFile = [ "data" ];
      command = ''
        ${mainProgram yq'build} ${optionalString (versionAtLeast (yq'build.version or "2") "2.8.0") "--indentless-lists "}--yaml-output -c . $dataPath > $out
      '';
    });
  };
}
